...
def message_or_file():
    values = enter_message()
    if len(values) == 3:  # If enter_message returns three values (console input)
        return values[0], values[1], None, values[2]
    else:  # If enter_message returns four values (file input)
        return values
